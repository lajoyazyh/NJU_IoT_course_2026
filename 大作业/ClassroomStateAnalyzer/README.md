...
- **⚡ 快速模式**（默认开启）：跳过数据增广，速度提升约 6 倍。关闭则使用增广策略，准确率略高
...
**性能优化**：
- **快速模式** `recognize_all_fast()`：跳过增广，所有人脸拼接为单个 batch 一次性预测，速度提升约 6 倍
- **模型预热** `warm_up()`：启动时做一次空预测，避免 TF 首次推理的 5~10 秒冷启动延迟
...
recognizer.warm_up()           # 预热模型（消除冷启动延迟）
...
# 多人快速识别（无增广，速度快 6 倍，推荐实时场景）
results = recognizer.recognize_all_fast(image_bgr, faces)
```
